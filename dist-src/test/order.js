...
describe('POST /Orders', () => {
  ...
  before(done => {
    _dbConnection.default.query('DELETE from products where name = \'cake Test\'').then(() => {
      done();
    }).catch(() => done());
  });
  before(done => {
    _dbConnection.default.query('DELETE from products where name = \'chin chin Test\'').then(() => {
      done();
    }).catch(() => done());
  });
  before(done => {
    _dbConnection.default.query('DELETE from products where name = \'donuts\'').then(() => {
      done();
    }).catch(() => done());
  });
  before(done => {
    ...
    _chai.default.request(_index.default).post('/api/v1/menu').send(product).set('Authorization', token1).then(res => {
  ...
  describe('POST /Order', () => {
    ...
    it('should order for one or more items', () => {
      ...
      return _chai.default.request(_index.default).post('/api/v1/orders').send(order).set('Authorization', token3).then(res => {
        res.should.have.status(201);
        res.body.should.be.a('object');
    ...
    it('should not post if at least one item quantity is larger than the quantity on the menu', () => {
      ...
      return _chai.default.request(_index.default).post('/api/v1/orders').send(order).set('Authorization', token3).then(res => {
        ...
        res.body.should.be.a('object');
        res.body.should.have.property('message').eql('not enough quantity for selected product(s)');
    ...
    it('should deny access if user is not logged in', () => {
      ...
      return _chai.default.request(_index.default).post('/api/v1/orders').send(order).then(res => {
  ...
  describe('GET /orders', () => {
    it('should get all orders', () => {
      return _chai.default.request(_index.default).get('/api/v1/orders').send().set('Authorization', token1).then(res => {
    ...
    it('should get order by orderId', () => {
      return _chai.default.request(_index.default).get(`/api/v1/orders/${orderId}`).send().set('Authorization', token1).then(res => {
        res.should.have.status(200);
        res.body.should.be.a('object');
        res.body.should.have.property('status').eql('success');
    ...
    it('should not get all orders if user is not administrators', () => {
      return _chai.default.request(_index.default).get('/api/v1/orders').send().set('Authorization', token3).then(res => {
        res.should.have.status(403);
        res.body.should.be.a('object');
    ...
    it('should not get order by Id if user is not administrators', () => {
      return _chai.default.request(_index.default).get(`/api/v1/orders/${orderId}`).send().set('Authorization', token3).then(res => {
        res.should.have.status(403);
        res.body.should.be.a('object');
        res.body.should.have.property('message').eql('Access Denied');
      }).catch(err => {
    ...
    it('should get all orders of a logged in user', () => {
      return _chai.default.request(_index.default).get(`/api/v1/orders/${userId}/my_orders`).send().set('Authorization', token3).then(res => {
        res.should.have.status(200);
        res.body.should.be.a('object');
        res.body.should.have.property('status').eql('success');
        res.body.should.have.property('message').eql('orders fetched successfully!');
    ...
    it('should get login user order by id', () => {
      return _chai.default.request(_index.default).get(`/api/v1/orders/${userId}/my_orders/${orderId}`).send().set('Authorization', token3).then(res => {
        res.should.have.status(200);
        res.body.should.be.a('object');
        res.body.should.have.property('status').eql('success');
        res.body.should.have.property('message').eql('orders fetched successfully!');
  ...
  describe('PUT /orders/:orderId', () => {
    it('should update order status', () => {
      ...
      return _chai.default.request(_index.default).put(`/api/v1/orders/${orderId}`).send(order).set('Authorization', token1).then(res => {
        res.should.have.status(200);
        res.body.should.be.a('object');
        res.body.should.have.property('status').eql('success');
    ...
    it('should not update status if user is not administrator', () => {
      ...
      return _chai.default.request(_index.default).put(`/api/v1/orders/${orderId}`).send(order).set('Authorization', token3).then(res => {
        res.should.have.status(403);
        res.body.should.be.a('object');
        res.body.should.have.property('message').eql('Access denied.');
      }).catch(err => {
