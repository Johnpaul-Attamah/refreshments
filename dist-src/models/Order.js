...
class Order {
  ...
  async placeOrder(input, userId) {
    try {
      const query = {
        ...
        values: [userId, input.recievingAddress, (0, _moment.default)(new Date()), (0, _moment.default)(new Date())]
      };
  ...
  async updateOrderStatus(status, orderId) {
    try {
      const result = await this.pool.query('UPDATE orders set status = $1, modified_date = $2 WHERE order_id = $3 RETURNING *', [status, (0, _moment.default)(new Date()), orderId]);
